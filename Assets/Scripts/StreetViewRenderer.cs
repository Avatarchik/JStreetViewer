...
{ 
	...
	IEnumerator GetMetaData()
	{
		...
		nextIDs = new string[linkCount];
		nextDegrees = new string[linkCount];
	...
	void DrawButtons()
	{
		...
		buttonModelList = new GameObject[nextDegrees.Length];
		for (int i = 0; i < nextDegrees.Length; i++)
		{
			buttonModelList[i] = Instantiate(buttonModel, transform.position, Quaternion.identity) as GameObject;
			buttonModelList[i].GetComponent<Button>().SetDegree(Convert.ToSingle(nextDegrees[i]) - pivotYaw);
			buttonModelList[i].GetComponent<Button>().SetPanoramaID(nextIDs[i]);
		}
	}
